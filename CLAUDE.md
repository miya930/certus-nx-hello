...
- ユーザーへの返答は、長い説明や設計の提案も含めて、全て日本語で書く。
  途中で英語に切り替えない。
- コードの識別子、コマンド、エラーメッセージは、元の表記のまま書いてよい。
...
- データシートはメーカーの資料で、再配布にあたるため、PDF も Markdown もコミットしない。
- データシートは、`add-datasheet` スキルの手順で各自の手元に用意する。
- 回路図と BOM のページは、図中の文字が崩れて変換されている。
  読み取れない場合は PDF の該当ページを確認する。

## リポジトリ構成

- 合成とシミュレーションは、リポジトリ外のローカルファイルに依存しない。
...
- PC によって変換結果が変わらないよう、依存の版は `uv lock --script` で作るロックファイルで固定し、スクリプトと同じフォルダに置く。
...
- 新しいプロジェクトの Makefile は `projects/managed_switch/Makefile` を元に作る。
...
- 英語の用語を、不自然な日本語に直訳しない。
  日本語に定着したカタカナ語があれば、それを使う。
  例えば port はポート、word はワード、bridge はブリッジ、address はアドレス、copy はコピーと書き、窓、語、橋渡し、番地、写すとは書かない。
  定着した言い方がない用語は、firmware image のように英語のまま書き、像のような訳語を作らない。
  コードのコメントにも、同じ決まりを当てる。
- 太字を多用しない。
...
- プロジェクトのドキュメントは、プロジェクトの README.md 1 つにまとめる。
- プロジェクトの README.md の先頭の `# ` の見出しには、フォルダ名ではなく、何を作るかがわかる名前を書く。
- 最終更新日は git の履歴でわかるため、ドキュメントに書かない。
- FPGA のプロジェクトの README.md は、冒頭に何を作るかを書き、続けて構成、設計、検証、回路規模、実行方法の見出しを置く。
...
- テストベンチは全てパスすること。
- コミットメッセージは英語で書く。
- プルリクエストのタイトルと説明は、確認の項目まで全て日本語で書く。
