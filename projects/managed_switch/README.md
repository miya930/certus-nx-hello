...
|---|---|---|---|
| `0x00000000` | 32 KB | 命令メモリ | EBR |
| `0x80000000` | 32 KB | データメモリ | EBR |
...
| 2 `cfgbus_port_stats` | 16n + 2、16n + 3 | 読み | ポート n の受信のバイト数とフレーム数 | `stats` |
| | 16n + 4、16n + 5 | 読み | ポート n の送信のバイト数とフレーム数 | `stats` |
| | 16n + 6 | 読み | ポート n のエラーの数 | `stats` |
| | 16n + 8 | 読み | ポート n の速度と状態 | `status` |
| | どれでも | 書き | 全ポートの数を取り込み、数え直す | `stats` |
...
| データメモリの残り | スタック。`0x80008000` から下へ伸びる |

今のファームウェアは、命令メモリの約 30 KB を使い、`.data` と `.bss` は持たない。
...
|---|---|
| `help` | コマンドの一覧を出す |
| `status` | ポートごとのリンクの状態を出す |
| `stats` | 前回の `stats` からの送受信の数とエラーの数を出す |
| `info` | スイッチコアのポート数、クロック、MAC テーブルの大きさを出す |
| `show` | 設定を出す |
...
- 命令メモリの 32 KB に収めるため、開発用のビルドもリリースと同じ最適化にする。
...
タイミングの余裕が一番少ないのは RGMII の送信側で、125 MHz に対して 142.8 MHz である。
...
ファームウェアは、命令メモリの 32 KB のうち約 29 KB を使う。
