...
次の表は、FPGA の中の全ての領域を、CPU から見たアドレスで示す。
FPGA の外とは、I/O のピンでつながる。
...
保存する設定の書式は次のとおりで、複数バイトの値はネットワークバイトオーダーで置く。
...
送信のずれは 2.00 ns では余裕が足りず、ping の応答のバイトのビット 1 が化けたため、4.00 ns にしている。
...
表には、時間がたった項目を消す仕組みもない。
そのため機器を別のポートに差し替えると、その機器へのフレームは元のポートに送られ続ける。

そこでファームウェアは、DP83867 のリンクが変わるたびに表を消し、再学習させる。
...
| Tab                        | コマンド、設定の名前、`help` を補完し、候補が複数あれば一覧を出す |
...
| コマンド                                         | 内容                                                                           |
| ------------------------------------------------ | ------------------------------------------------------------------------------ |
| `help`                                           | コマンドと、引数を付けた形の一覧を出す                                         |
| `set help` のように、コマンドの後に `help`       | そのコマンドの形と、そのまま打てる例を出す                                     |
| `status`                                         | ポートごとのリンクの状態と、直近 1 秒の送受信の速さを出す                      |
| `status rgmii`、`status rmii`、`status cpu`      | 1 つのポートの詳しい状態と、送受信とエラーの数の内訳を出す                     |
...
| 処理                                                           | 周期   | 型              |
| -------------------------------------------------------------- | ------ | --------------- |
| CPU のポートに届いたフレームを 1 つ処理し、応答する            | 毎回   | `Host`          |
...
割り込みは使わず、全ての処理をメインループの中で順に行う。
そのため、1 つの処理が長くかかると、ほかの処理はその間待たされる。
CPU のポートの受信は、1 周に 1 フレームだけ処理する。
フレームが届き続けても、コンソールや周期のある処理が止まらないようにするためである。
周期のある処理は、メインループが遅れても遅れた分を取り戻さず、遅れた時刻から次の周期を数える。

端末への出力は、UART の送信の FIFO の 64 バイトを超えると、115200 bps で送り終わるのを待つ。
例えば 1 KB を出すコマンドは、約 80 ms の間、メインループを止める。

何もしていないとき、メインループは 1 秒に約 4400 回まわる。
CPU のポートで 1000 バイトの ping を 1 つ処理すると、その回のメインループは約 8 ms かかる。
この ping を 1 秒に 100 回送ると、メインループは 1 秒に約 1100 回に落ち、ping の約 3% が失われる。
...
- 使わない unwind table の `.eh_frame` を、`firmware/memory.x` で捨てる。
...
| レベル | 内容                                                                                                                                  |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| info   | 設定の読み込みと保存、設定を反映したときの IP アドレス、DP83867 のリンクの変化と MAC アドレステーブルの消去、RMII の REF_CLK のロック |
...
タイミングの余裕が一番少ないのは RGMII の送信側で、125 MHz に対して 143.8 MHz である。
...
ファームウェアは `firmware/` の中で書き込む。
`cargo run` は SPI Flash と命令メモリの両方に書き、電源を入れ直しても残る。
