...
スイッチは、ボードの DP83867 と、PMOD につないだ 2 つの LAN8720 の間でフレームを中継する。
...
スイッチのポートは 4 つある。
ボードの DP83867 には RGMII で、PMOD の J6 と J8 につないだ 2 つの LAN8720 のモジュールには RMII でつながる。
...
RMII のポートは、それぞれの LAN8720 のモジュールから来る 50 MHz の REF_CLK で動く。
...
3 つの Ethernet のポートが全て 100 Mbps で受信し続けると、スイッチコアの処理が追いつかない。
...
続けて、3 つの Ethernet のポートから 1 つずつフレームを送り、送信元以外の全てのポートからそのまま出ることを確かめる。
送信元のポートへフレームを返さないことも確かめる。
...
タイミングの余裕が一番少ないのは RGMII の送信側で、125 MHz に対して 142.9 MHz である。
配置によってはこの余裕がなくなるため、Makefile で配置のシードを決めている。
...
LAN8720 のモジュールは、PMOD の J6 と J8 に次のようにつなぐ。
J5 は、NEORV32 の JTAG のために空けておく。
モジュールの電源は、つないだ PMOD の 3.3 V と GND から取る。
J8 の I/O の電源は、J48 を既定のままにして 3.3 V にしておく。
...
3 つの RJ45 につないだ機器は、スイッチを通して互いに通信できる。
